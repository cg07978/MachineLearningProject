...
plt.plot(alphas, train_scores, label='No Duplicate Set Ridge Training Score')
plt.plot(alphas, test_scores, label='No Duplicate Set Ridge Testing Score')
plt.legend()
plt.figure()

alphas = np.linspace(0, 160, 1000)

rr_scores = [cross_val_score(Ridge(alpha=a), X, y, cv=5).mean() for a in alphas]

plt.plot(alphas, rr_scores, label='No Duplicate Set Ridge 5-fold CV Score')
plt.legend()
